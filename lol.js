...
async function run() {
  try {
    ...
    const productCollection = client.db('troyal_electro').collection('Products');
    const orderCollection = client.db('troyal_electro').collection('order');
    const userCollection = client.db('troyal_electro').collection('users');
    const adminCollection = client.db('troyal_electro').collection('admin');
    const paymentCollection = client.db('troyal_electro').collection('payments');
    const bookinguCollection = client.db('troyal_electro').collection('products');
    ...
    */
    ...
    app.get('/doctor', verifyJWT, verifyAdmin, async (req, res) => {
      const doctors = await adminCollection.find().toArray();
      res.send(doctors);
    })

    app.post('/doctor', verifyJWT, verifyAdmin, async (req, res) => {
      const doctor = req.body;
      const result = await adminCollection.insertOne(doctor);
      res.send(result);
    });
...
app.get('/', (req, res) => {
  res.send('Welcome to Trayal Electro!')
})

app.listen(port, () => {
  console.log(`Troyal Electr App listening on port ${port}`)
})